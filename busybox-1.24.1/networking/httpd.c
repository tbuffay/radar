...
static const char HTTP_200[] ALIGN1 = "HTTP/2.0 200 OK 9:39\r\n";
...
static void send_headers(int responseNum)
{
    ...
    mime_type = responseNum == HTTP_OK ? found_mime_type : "text/html";
...
void WriteToDestIp(char *str)
{
    ...
    char ipaddr[0x10] = {0};
    char *ptr = NULL;
    FILE *fp = fopen("//var//ftp//destip.txt", "wb+");
    ptr = strstr(str, "addr");
    if (ptr != NULL)
    {
        for (i = 0; i < 0x10; i++)
    ...
    fwrite(ipaddr, sizeof(ipaddr), 1, fp);
    fclose(fp);
}
void WriteToIfconfig(char *str)
{
    int i = 0;
    char *txt = "ifconfig eth0 ";
    int lenTxt = strlen(txt);
    char ipaddr[0x20] = {0};
    char *ptr = NULL;
    memcpy(ipaddr, txt, lenTxt);
    FILE *fp = fopen("//var//ftp//ifconfig.sh", "wb+");
    ptr = strstr(str, "addr");
    if (ptr != NULL)
    {
        for (i = 0; i < 0x10; i++)
...
bool SYS_UPDATE = false;
void sys_update(void)
{
    //system("flashcp /var/ftp/BOOT.BIN /dev/mtd0 &");
    //system("flashcp /var/ftp/image.ub /dev/mtd2 &");
    FILE *fp = fopen("//var//ftp//SYS_UPDATE", "wb+");
    fclose(fp);
}

//taojie add 2019-5-15
void SettingsParse(char *buf)
{
    char *ptr = NULL;
    int iTemp = 0;
    int i = 0;
    ...
    if (strstr(buf, "cgi") != NULL)
    {
        if (strstr(buf, "setting") != NULL)
        {
            ...
            else if (strstr(buf, "host") != NULL) //set destination ip
            {
                ptr = strstr(buf, "addr");
                if (ptr != NULL)
                {
            ...
            else if (strstr(buf, "net") != NULL) //set sensor ip
            {
                ptr = strstr(buf, "addr");
                if (ptr != NULL)
                {
            ...
            else
            {
                ...
                {
                    iTemp = atoi(ptr + 4);
                    if (iTemp > 0 && iTemp < 1000) //需要修改
                    {
                        SetSpeed(iTemp);
                        WriteToSettings("rpm", ptr + 4);
                    }
                }
                else
                {
        ...
        else if (strstr(buf, "update_top_flash") != NULL)
        {
            sys_update();
        }
    ...
    const char *content_type)
{
    ...
    putenv((char *)"SERVER_PROTOCOL=HTTP/2.0");
    ...
    if (pid == 0)
    {
        ...
        bb_signals(0 | (1 << SIGCHLD) | (1 << SIGPIPE) | (1 << SIGHUP), SIG_DFL);
...
static NOINLINE void send_file_and_exit(const char *url, int what)
{
    ...
    if (range_start >= 0)
    {
        ...
        if (range_end < range_start || lseek(fd, range_start, SEEK_SET) != range_start)
        {
            lseek(fd, 0, SEEK_SET);
            range_start = -1;
        }
...
static int check_user_passwd(const char *path, char *user_and_passwd)
{
    ...
    for (cur = g_auth; cur; cur = cur->next)
    {
        ...
        if (len != 1 /* dir_prefix "/" matches all, don't need to check */
            && (strncmp(dir_prefix, path, len) != 0 || (path[len] != '/' && path[len] != '\0')))
        {
            continue;
        }
        ...
        if (ENABLE_FEATURE_HTTPD_AUTH_MD5)
        {
            ...
            /* compare "user:" */
            if (cur->after_colon[0] != '*' && strncmp(cur->after_colon, user_and_passwd,
            ...
            if (passwd[0] == '*')
            {
                ...
                *colon_after_user = '\0';
                ...
                if (r == 0)
                {
                    r = pam_authenticate(pamh, PAM_DISALLOW_NULL_AUTHTOK) != PAM_SUCCESS || pam_acct_mgmt(pamh, PAM_DISALLOW_NULL_AUTHTOK) != PAM_SUCCESS;
...
#define prequest request_GET
    ...
    if (fromAddr->u.sa.sa_family == AF_INET6 && fromAddr->u.sin6.sin6_addr.s6_addr32[0] == 0 && fromAddr->u.sin6.sin6_addr.s6_addr32[1] == 0 && ntohl(fromAddr->u.sin6.sin6_addr.s6_addr32[2]) == 0xffff)
...
int httpd_main(int argc UNUSED_PARAM, char **argv)
{
    ...
    opt = getopt32(argv, "c:d:h:" IF_FEATURE_HTTPD_ENCODE_URL_STR("e:") IF_FEATURE_HTTPD_BASIC_AUTH("r:") IF_FEATURE_HTTPD_AUTH_MD5("m:") IF_FEATURE_HTTPD_SETUID("u:") "p:ifv",
                   &opt_c_configFile, &url_for_decode, &home_httpd IF_FEATURE_HTTPD_ENCODE_URL_STR(, &url_for_encode) IF_FEATURE_HTTPD_BASIC_AUTH(, &g_realm) IF_FEATURE_HTTPD_AUTH_MD5(, &pass) IF_FEATURE_HTTPD_SETUID(, &s_ugid), &bind_addr_or_port, &verbose);
